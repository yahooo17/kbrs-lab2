...
public abstract class RSAKey implements  RSABaseInterface
{
    ...
    public final boolean isPositive(BigInteger number) {
        return (number.compareTo(RSAConstants.ZERO) > 0);
    }
    ...
    protected byte[] toByteArray(BigInteger x, int numBytes) {
        if (x.compareTo(RSAConstants.TWO_FIFTY_SIX.pow(numBytes)) >= 0) {
        ...
        for (int power = numBytes; power >= 0; power--) {
            divAndRem = x.divideAndRemainder(RSAConstants.TWO_FIFTY_SIX.pow(power));
    ...
    protected BigInteger toInteger(byte[] X) {
        BigInteger x = RSAConstants.ZERO;

        for (int i = 0; i < X.length; i++) {
            x = x.add(BigInteger.valueOf(X[i]).multiply(RSAConstants.TWO_FIFTY_SIX.pow(X.length - 1 - i)));
        }

        return x;
    }

    public abstract List<Byte> use(String source, String destination);
